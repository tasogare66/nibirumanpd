-- title:  Nibiruman:2080(Playdate)
-- author: tasogare66
-- desc:   intense arcade-style shooter
-- script: lua

-- https://github.com/tasogare66/nibirumanpd
...
SCR_WIDTH=playdate.display.getWidth()   --200,orig:240
SCR_HEIGHT=playdate.display.getHeight() --120,orig:136
...
function print_hcenter(str,y,col,fixed,scl)
  ...
  scl = scl or 1
  local width=print_txt(str,0,-48,col,fixed,scl)
  local x=(SCR_WIDTH-width)//2
  print_txt(str,x,y,col,fixed,scl)
  return x
end
function print_dbg(str,x,y)
  print_txt(str,x,y,15,true,1,true)
end
...
function Boss:drw_info()
  if self.appear_flag then
    ...
    print_txt(wstr,x+ofsx,y+ofsy,random(1,15))
    print_txt(wstr,x,y,6)
  end
...
mode_game.draw1 = function(self)
  ...
  c=clamp(c,1,3)
  local col={6,15,11}
  print_txt(string.format("%5.2fx",self.multiplier),180+6*2,8,col[c],true)
  print_txt(string.format("% 10d",self.score),180-6,0,15,true)
  ...
  elseif self.state==self.State_Over then
    ...
    if r%2==0 then
      ...
      print_txt(gov,x+1,y+1,3)
      print_txt(gov,x-1,y-1,3)
      print_txt(gov,x,y,15)
    end
  end
  if _DEBUG then
...
function mode_title:draw1()
  ...
  print_txt(t,x-1,y,12,false,2)
  print_txt(t,x,y-1,12,false,2)
  ...
  print_txt("REPLAY",x,80,CURSOR==1 and rc or (Input:exists_log()and 15 or 10))
  print_hcenter("HIGH SCORE",2,6,false,1)
  print_hcenter(string.format("%d",HISCORE),10,15,true,1)
  print_txt(GAME_VER,4,SCR_HEIGHT-16,7,false)
  print_txt("@tasogare66 2020",4,SCR_HEIGHT-8,7,false)
end
